...
eventsDF = (
        ...
        .option("cloudFiles.schemaLocation", schema_dir)
        .option("inferColumnTypes", True)
        .option("cloudFiles.schemaHints", "eventId string, timeReceived timestamp")
        .load(data_dir)
    )

tumblingWindows = eventsDF.withWatermark("timeReceived", "0 minutes").groupBy("eventId", window("timeReceived", "1 seconds")).agg((count("eventId")).alias("count"), collect_list("timeReceived").alias("timelist")).withColumn("ingesttime", current_timestamp()) #.count()
...
# MAGIC %sql
# MAGIC select * from yas_catalog.yas_db.windowing
